...
from largo.drivers.ni.ni_motion_controller import NIDAQMotionController, NIDAQAxis #Maybe figured this out??? -CE 020123
...
class FSM_2D():
    ...
    def __init__(self, x_ch='Dev1/ao1', y_ch='Dev1/ao1', ctr_ch='Dev1/ctr0', XperV = 10.9472683, YperV = 7.37113658,
     acq_rate = 20e3, taskNumb = 0): #Was DAQ1, not Dev1, but this allowed a move command to work
        # Create axis objects; limits are -10 to 10V
        self.initparams = [x_ch, y_ch, ctr_ch, XperV, YperV]
    ...
    def __enter__(self):
        ...
        self.daq_controller = NIDAQMotionController(self.ctr_ch, self.acq_rate, 
        {'x': self.x_axis, 'y': self.y_axis,}, taskNumb = self.taskNumb)#, ao_smooth_steps=5000) #already default smooth arg
        print('FSM Connected')
