...
class NIDAQMotionController():
    def __init__(self, ctr_ch, acq_rate, axes, ao_smooth_steps=5000, taskNumb = 1):
        ...
        #added by ian: 
        self.taskNumb = taskNumb
        self.dev_name = 'Dev1'
        self.system = nidaqmx.system.System.local()
        self.device = self.system.devices[self.dev_name]
        #https://nidaqmx-python.readthedocs.io/en/latest/device.html?highlight=system%20device#module-nidaqmx.system.device
        self.apdterm = 'PFI5'
        self.APD_CHANNEL = "/" + self.dev_name + "/" + self.apdterm


    def __enter__(self):
        '''Actually initialize and connect to device'''
        taskName= f'NIDAQMotionController_AO{self.taskNumb}'
        self.ao_motion_task = nidaqmx.Task(taskName)
       # print(f"Created task: {taskName}")
        for a in self.axes:
            min_max_dict = {}
    ...
    def new_ctr_task(self, ctr_ch):
        ...
        self.current_counter_task.ci_channels.all.ci_count_edges_term = self.APD_CHANNEL #Hard-coded to take all counts channel from Jasper #TODO: Un-hardcode this?
    ...
    def line_scan(self, init_point, final_point, steps, pts_per_step=1):
        ...
        if timing == True: 
            print(f"Acq Rate: {self.acq_rate}")
            st = time.time()
        ...
        if timing == True: 
            finalT = time.time()-st 
            print(f"took {finalT} to final up")
